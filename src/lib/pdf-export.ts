...
// Function to share individual documents with better mobile support
...
// Enhanced mobile sharing with better document handling
export async function shareLoanMobile(loan: LoanData, docs: any[] = []) {
  try {
    ...
    // Separate document types for better mobile compatibility
    const imageFiles = docFileObjs.filter(docFile => {
      const fileType = docFile.file.type;
      return fileType.startsWith('image/') || fileType.includes('jpeg') || fileType.includes('jpg') || fileType.includes('png');
    });
    
    const pdfDocuments = docFileObjs.filter(docFile => {
      const fileType = docFile.file.type;
      return fileType.includes('pdf');
    });
    ...
    try {
      // Strategy 1: Try sharing main PDF with images (works on newer devices)
      const allFiles = [pdfFile, ...imageFiles.map(f => f.file)];
      if (navigator.canShare && allFiles.length > 1) {
        const canShareAll = await navigator.canShare({ files: allFiles });
        if (canShareAll) {
          await navigator.share({ 
          ...
          // Offer to share PDF documents separately if any
          if (pdfDocuments.length > 0) {
            setTimeout(async () => {
              const sharePDFs = confirm(`Share ${pdfDocuments.length} additional PDF documents?`);
              if (sharePDFs) {
                try {
                  await navigator.share({
                    title: 'Additional PDF Documents',
                    text: `Additional documents for loan ${loan.id}`,
                    files: pdfDocuments.map(f => f.file)
                  });
                  toast.success('Additional PDFs shared!');
                } catch (e) {
                  toast.error('Could not share additional PDFs');
                }
              }
            }, 3000);
          }
      ...
      // Strategy 2: Share main PDF only, then offer images
      const canSharePDF = await navigator.canShare({ files: [pdfFile] });
      if (canSharePDF) {
        await navigator.share({ 
          title: `Loan Application - ${loan.id}`,
          text: loanMessage,
        ...
        // Offer to share images separately
        if (imageFiles.length > 0) {
          setTimeout(async () => {
            ...
            if (shareImages) {
              try {
                ...
                toast.success('Document images shared!');
              } catch (e) {
        ...
        toast.success('PDF shared! Additional documents can be shared separately.');
        return;
      }
      
      // Strategy 3: Share images only if PDF sharing fails
      if (imageFiles.length > 0) {
        const canShareImages = await navigator.canShare({ files: imageFiles.map(f => f.file) });
        if (canShareImages) {
          await navigator.share({ 
          ...
          toast.success(`Shared ${imageFiles.length} document images!`);
          toast.info('Main PDF can be generated separately if needed.');
          return;
        }
      }
